...
TestConfig (static config)
ApiAssertions (static utils)
RequestBuilder (static utils)
```

### Слои фреймворка

1. **Base Layer** - базовая конфигурация и аутентификация
2. **Config Layer** - централизованные константы
3. **Utils Layer** - переиспользуемые утилиты
4. **Test Layer** - конкретные тестовые сценарии
